...
int calculateChildNodesCnt(std::shared_ptr<TreeNode>& treeNode)
{
    ...
    treeNode->childrenLeftCnt = calculateChildNodesCnt(treeNode->left);
    treeNode->childrenRightCnt = calculateChildNodesCnt(treeNode->right);

    return treeNode->childrenLeftCnt + treeNode->childrenRightCnt + 1;
}
